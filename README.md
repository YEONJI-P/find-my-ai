...
결과 페이지 진입 또는 "다시 시도하기" 클릭 시마다 Gemini API가 호출될 수 있어, 불필요한 중복 요청을 방지할 방법을 검토함.

| 방법 | 적용 위치 | 특징 |
|------|----------|------|
| **A. sessionStorage 캐시** | 클라이언트 | 입력값 조합(occupation + interests + concern + aiName)을 키로 해시 후 캐시. 탭 닫으면 자동 만료. 구현 난이도 낮음. |
| **B. API route rate limit** | 서버 | IP 기반 N초/M회 초과 시 429 반환. 인메모리 Map으로 구현 가능(서버 재시작 시 초기화 한계). |
| **C. 요청 중 버튼 비활성화** | 클라이언트 | loading 상태에서 재요청 UI 자체를 비활성화. 구현 난이도 최저, 페이지 재방문은 막지 못함. |

**결정:** A(sessionStorage) + C(버튼 비활성화) 조합 적용 예정. B는 인프라 없이 완전한 방어가 어려워 우선순위 낮음.
...
- **제거 시:** 서비스 종료된 도구를 추천하는 UX상 문제를 방지할 수 있으나, 매칭 데이터·가중치 정리 작업 필요
- **유지 시:** 국내 로컬 특화 AI 포지셔닝 케이스로 매칭 로직 테스트에 활용 가능하나, 실사용자에게 노출되지 않도록 결과 표시 제어 필요

**향후 결정:** 서비스 종료가 공식 확정되면 목록에서 제거하고, 대체 국내 AI 도구(예: 뤼튼 확장, ClovaStudio 등) 편입 검토.
